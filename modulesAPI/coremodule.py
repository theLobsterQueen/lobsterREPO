...
def get_component(comp_name, entID) :
    for comp in comps :
        if comp.__class__.__name__ == comp_name.title() and comp.entity.id == entID :
            return comp
...
class Component() :
    ...
    def __repr__(self) :
        return f"COMPONENT OF TYPE {self.__class__.__name__.upper()} IN ENTITY {self.entity}!"

class Transform(Component) :
    def __init__(self, input_entity, position = [], rotation = [], scale = []) :
        super().__init__("Transform", input_entity)
...
class Mesh(Component) :
    def __init__(self, input_entity, mesh_name = "", tex_name = "") :
        super().__init__("Mesh", input_entity)
...
class Light(Component) :
    def __init__(self, input_ent, input_color = []) :
        super().__init__("Light", input_ent)
